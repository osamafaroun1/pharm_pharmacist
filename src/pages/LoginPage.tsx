...
// ====================== Zod Schema (محدث - 4 حقول غير مطلوبة) ======================
const registerSchema = z.object({
    firstName: z
        ...
        .refine(
            val => val.length > 0,
            'الاسم الأول لا يمكن أن يكون مسطرات أو مسافات فقط'
        ),
    ...
    lastName: z
        ...
        .refine(
            val => val.length > 0,
            'الاسم الأخير لا يمكن أن يكون مسطرات أو مسافات فقط'
        ),
    ...
    const handleRegister = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setRegForm({
                ...
                password: '', pharmacyName: '', pharmacyLocation: '',
                pharmacyLocationDetails: '', licenseImage: '', licenseFileName: '',
            });
        } catch (err: any) {
                    ...
                    {tab === 'login' && (
                        <form onSubmit={handleLogin}>
                            <div className="input-wrap">
                                ...
                                <input className="input-field" placeholder="09xxxxxxxx" value={loginForm.identity} onChange={e => setLoginForm({ ...loginForm, identity: e.target.value })} />
                            </div>
                            <div className="input-wrap" style={{ marginBottom: 24 }}>
                                <label className="input-label">كلمة المرور *</label>
                                <input className="input-field" type="password" placeholder="••••••••" value={loginForm.password} onChange={e => setLoginForm({ ...loginForm, password: e.target.value })} />
                            </div>

                            {serverError && <p className="error-text" style={{ textAlign: 'center', marginBottom: 16 }}>{serverError}</p>}
                        ...
                        <form onSubmit={handleRegister}>
                            ...
                            <div style={{ background: 'var(--s2)', borderRadius: 'var(--r2)', padding: '14px 16px', marginBottom: 16 }}>
                                ...
                                <div className="input-row" style={{ marginBottom: 12 }}>
                                    <div>
                                        ...
                                        <input className="input-field" value={regForm.firstName} onChange={e => setRegForm({ ...regForm, firstName: e.target.value })} />
                                        {errors.firstName && <p className="error-text">{errors.firstName}</p>}
                                    </div>
                                    <div>
                                        <label className="input-label">الاسم الأخير *</label>
                                        <input className="input-field" value={regForm.lastName} onChange={e => setRegForm({ ...regForm, lastName: e.target.value })} />
                                ...
                                <div className="input-row" style={{ marginBottom: 12 }}>
                                    <div>
                                        ...
                                        <input className="input-field" placeholder="09xxxxxxxx" value={regForm.phone} onChange={e => setRegForm({ ...regForm, phone: e.target.value })} />
                                        {errors.phone && <p className="error-text">{errors.phone}</p>}
                                    </div>
                                    <div>
                                        <label className="input-label">هاتف ثابت</label>
                                        <input className="input-field" value={regForm.landline} onChange={e => setRegForm({ ...regForm, landline: e.target.value })} />
                                    </div>
                                </div>
                                <div className="input-wrap">
                                    <label className="input-label">البريد الإلكتروني</label>
                                    <input type="email" className="input-field" value={regForm.email} onChange={e => setRegForm({ ...regForm, email: e.target.value })} />
                            ...
                            <div style={{ background: 'var(--s2)', borderRadius: 'var(--r2)', padding: '14px 16px', marginBottom: 16 }}>
                                ...
                                <div className="input-wrap">
                                    ...
                                    <input className="input-field" value={regForm.pharmacyName} onChange={e => setRegForm({ ...regForm, pharmacyName: e.target.value })} />
                                    {errors.pharmacyName && <p className="error-text">{errors.pharmacyName}</p>}
                                </div>
                                <div className="input-wrap">
                                    <label className="input-label">موقع الصيدلية (المنطقة) *</label>
                                    <input className="input-field" value={regForm.pharmacyLocation} onChange={e => setRegForm({ ...regForm, pharmacyLocation: e.target.value })} />
                                    {errors.pharmacyLocation && <p className="error-text">{errors.pharmacyLocation}</p>}
                                </div>
                                <div className="input-wrap">
                                    <label className="input-label">تفاصيل الموقع</label>
                                    <textarea className="input-field" rows={3} value={regForm.pharmacyLocationDetails} onChange={e => setRegForm({ ...regForm, pharmacyLocationDetails: e.target.value })} />
                                </div>
                            </div>

                            {/* شهادة الصيدلية - التصميم القديم تماماً */}
                            <div style={{ background: 'var(--s2)', borderRadius: 'var(--r2)', padding: '14px 16px', marginBottom: 16 }}>
                                ...
                                {!regForm.licenseImage ? (
                                    ...
                                    <div style={{ border: '2px solid var(--p)', borderRadius: 'var(--r2)', padding: 12, background: 'var(--pf)', display: 'flex', alignItems: 'center', gap: 12 }}>
                                        ...
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            <div style={{ fontWeight: 600, fontSize: 13, color: 'var(--pd)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            ...
                            <div className="input-wrap" style={{ marginBottom: 24 }}>
                                <label className="input-label">كلمة المرور *</label>
                                <input type="password" className="input-field" placeholder="8 أحرف على الأقل" value={regForm.password} onChange={e => setRegForm({ ...regForm, password: e.target.value })} />
